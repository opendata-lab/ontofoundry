...
- 文件读写只在当前会话工作区目录、以及「运行时上下文」声明的可写临时目录内进行；不要访问共享运行时根 `/dataagent_runtime`、宿主 HOME、其他会话目录或父级目录；
- 可写临时目录只放中间过程文件，用户看不到也下载不了；最终交付文件必须写入工作区 `output/`；
- `.claude/` 是运行时状态目录，仅用于 Pi 会话和已启用 Skills，不作为业务文件读写区；
