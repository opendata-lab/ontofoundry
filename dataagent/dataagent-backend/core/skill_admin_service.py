...
def _legacy_provider_settings(payload: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
    ...
    if provider_id:
        ...
        if model:
            ...
            # A provider selected through the deployment environment is an
            # explicit runtime choice.  Fresh installations have no registry
            # row yet, so carry that intent into the initial provider row;
            # credential/model validation below still decides whether it is
            # actually usable.
            target["provider_enabled"] = True
    return legacy
...
async def _run_model_detection(
    *,
    ...
    del provider_id, supports_partial_messages
    endpoint = str(base_url or "https://api.anthropic.com").rstrip("/")
    if not endpoint.endswith("/v1/messages"):
        endpoint += "/v1/messages"
    token = str(auth_token or api_key).strip()
    headers = {
    ...
    try:
        async with httpx.AsyncClient(timeout=MODEL_DETECTION_TIMEOUT_SECONDS) as client:
            response = await client.post(
                ...
                json={
                    "model": model,
                    "max_tokens": 8,
                    "messages": [{"role": "user", "content": "reply model-service-ok"}],
                },
            )
