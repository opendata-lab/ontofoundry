...
class AgentSlashCommandsResponse(BaseModel):
    # Authoritative slash-command names derived from the agent profile.
...
class AgentEventRecord(BaseModel):
    seq_id: int
    turn_index: int = 0
    record_type: Literal["agent_event", "error"]
...
class AgentEventPageResponse(BaseModel):
    ...
    next_after_id: int
    has_more: bool
    records: List[AgentEventRecord] = Field(default_factory=list)
