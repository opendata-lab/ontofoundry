...
先结论：本页只记录 OpenDataWorks 通用平台业务规则和例外口径；查询方法、SQL 验证和执行由通用问数 SQL 技能处理。领域专属规则交给对应领域语义技能。
...
- 同义词：血缘关系、上游下游
- 规则：查询血缘时必须用 `db_name + table_name` 唯一定位目标表；只给 `table_name` 时必须先补充 `db_name`，在用户确认前不执行精确查询、模糊搜索或候选表推荐。
