...
def _run_via_topic_task(base_url: str, scenario: Scenario, provider_id: str, model: str, timeout_seconds: int, mode: str) -> dict[str, Any]:
    ...
    tool_outputs = [
        (record.get("data") or {}).get("output")
        for record in agent_records
        if str(record.get("record_type") or "") == "agent_event"
        and str(record.get("event_type") or "") == "tool.completed"
    ]
    ...
    return {
        ...
        "passed": passed,
        "reasons": reasons,
        "agent_event_count": len(agent_records),
        "agent_event_types": [str(record.get("event_type") or record.get("record_type") or "") for record in agent_records],
        "conversation": [
            {
