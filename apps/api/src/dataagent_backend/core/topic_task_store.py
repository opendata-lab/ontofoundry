...
from dataagent_backend.core.auth import is_auth_enabled
...
from dataagent_backend.core.task_status import to_downstream_status
from dataagent_backend.core.topic_workspace import delete_topic_workspace
...
def _project_agent_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Project platform ``agent_event`` records into persisted chat blocks."""
...
class TopicTaskStore:
                ...
                cur.execute(
                    ...
                    LEFT JOIN (
                        SELECT topic_id,
                               ...
                               (ARRAY_AGG(
                                   COALESCE(NULLIF(content, ''), '') ORDER BY seq_id DESC
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    SET current_task_status = 'running',
                        updated_at = CURRENT_TIMESTAMP
                    FROM da_agent_task k
    ...
    def append_permission_decision_record(
        ...
        *,
        ...
        """Append a durable permission decision if the request is still pending."""
        pending = self.get_pending_permission_request(task_id)
    ...
    def append_question_answer_record(
        ...
        *,
        ...
        """Append a durable question answer if the request is still pending."""
        pending = self.get_pending_question_request(task_id)
        if not pending or str(pending.get("request_id") or "") != str(request_id or ""):
            return False
        self.append_agent_record(
        ...
        *,
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    INSERT INTO da_agent_event_record (topic_id, task_id, turn_index, record_type, event_type, data)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    """,
                    (
    ...
    def append_agent_record(
        ...
        *,
        ...
        """Persist one AgentEvent or internal interaction-control record."""
        self._ensure_ready()
        env = envelope or {}
        conn = self._connect(database=self._schema_name())
        try:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO da_agent_event_record
    ...
    def _delete_task_content_deltas(self, task_id: str) -> int:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    DELETE FROM da_agent_event_record
                    WHERE task_id = %s
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    WHERE task_id = %s
                      AND id > %s
                      AND record_type IN ('agent_event', 'error')
                    ORDER BY id ASC
                    LIMIT %s
                    """,
    ...
    def _load_task_history_views(self, conn, task_ids: list[str]) -> dict[str, dict[str, Any]]:
        ...
        agent_records: dict[str, list[dict[str, Any]]] = {task_id: [] for task_id in task_ids}
            ...
            for row in cur.fetchall() or []:
                ...
                if isinstance(raw_data, str):
                    try:
                        ...
                        raw_data = {}
                data = raw_data or {}
                agent_records.setdefault(task_id, []).append({
                    "seq_id": int(row.get("id") or 0),
                    "turn_index": int(row.get("turn_index") or 0),
                    "record_type": str(row.get("record_type") or ""),
                    "event_type": row.get("event_type"),
        ...
        for task_id in task_ids:
            records = agent_records.get(task_id) or []
            if records:
                view = _project_agent_records(records)
            else:
