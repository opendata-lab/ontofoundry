            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    ORDER BY CASE category
                                 WHEN 'root' THEN 0 WHEN 'reference' THEN 1
                                 WHEN 'scripts' THEN 2 WHEN 'assets' THEN 3 ELSE 4
                             END,
                             relative_path
                    """
                )
        ...
        try:
            with conn.cursor() as cur:
                ...
                if existing:
                    ...
                    document_id = int((cur.fetchone() or {})["id"])
                    version_no = 1
                ...
                version_id = int((cur.fetchone() or {})["id"])
                cur.execute(
                    """
                    UPDATE da_skill_document
                    SET file_name = %s,
