"""Pi data plane adapter — runs one DataAgent turn on a Node Pi Cell over stdio.
...
no second copy of the event log to keep durable — events land in the platform
event store through :class:`core.agent_event_writer.AgentEventWriter`.
...
def resolve_cell_command(cfg: Any = None) -> list[str]:
    ...
    letting a spawn failure surface as an opaque OSError.
    """
...
def _stdio_frame_limit() -> int:
    ...
    Shares agent_max_buffer_size_bytes with the Pi transport and the sandbox
...
async def execute_pi_run(
    ...
    *,
    ...
    ``writer`` receives every neutral platform AgentEvent.
