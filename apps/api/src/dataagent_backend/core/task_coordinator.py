...
from dataagent_backend.core.task_executor import TaskExecutionInput, execute_task_stream
from dataagent_backend.core.task_submission_service import (
    compute_next_run_at,
    current_utc_naive,
    submit_message_task,
)
from dataagent_backend.core.topic_files import diff_generated_files, snapshot_workspace_state
from dataagent_backend.core.topic_task_store import TopicTaskStore, get_topic_task_store
...
class TaskCoordinator:
        ...
        try:
            async with self._semaphore:
                ...
                result = await execute_task_stream(
                    ...
                    emit=lambda record: self._persist_emitted_agent_record(
                        topic_id=topic_id,
                        task_id=task_id,
                        record=record,
                    ),
    ...
    def _persist_emitted_agent_record(self, *, topic_id: str, task_id: str, record: dict[str, Any]) -> None:
        # Sandbox runners may relay already-normalized platform records. Engine
        # events must never be persisted or exposed at this boundary.
        if not isinstance(record, dict):
            return
        record_type = str(record.get("record_type") or "").strip()
        if record_type not in {"agent_event", "error"}:
            return
        try:
        ...
        if not isinstance(data, dict):
            data = {"value": data}
        self.store.append_agent_record(
            task_id=task_id,
            topic_id=topic_id,
