...
from dataagent_backend.core.agent_visibility import agent_visible_to
from dataagent_backend.core.auth import AuthIdentity, is_auth_enabled, resolve_identity
from dataagent_backend.core.followup_suggestions import generate_followup_suggestions
from dataagent_backend.core.permission_gate import normalize_permission_decision
...
from dataagent_backend.core.task_submission_service import (
    compute_next_run_at,
    current_utc_naive,
    submit_message_task,
)
from dataagent_backend.core.topic_files import TopicFileError, list_files, safe_workspace_file, save_upload
...
# Message-level wording, deliberately broader than the task vocabulary: it also
...
async def api_list_agent_events(
    ...
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    records = store.list_agent_records(task_id=task_id, after_id=after_id, limit=limit + 1)
    has_more = len(records) > limit
    page = records[:limit]
    next_after_id = int(page[-1]["seq_id"]) if page else after_id
    return AgentEventPageResponse(
        task_id=task_id,
        task_status=str(task.get("task_status") or "waiting"),
        after_id=after_id,
        next_after_id=next_after_id,
        has_more=has_more,
        records=[AgentEventRecord.model_validate(r) for r in page],
    )
...
async def api_stream_agent_events(task_id: str, request: Request, after_id: int = Query(default=0, ge=0)):
    ...
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    logger.info(
        "agent_events.stream.open task_id=%s topic_id=%s after_id=%s task_status=%s",
        task_id,
    ...
    return StreamingResponse(
        _stream_agent_events(task_id=task_id, after_id=after_id, context=context),
...
async def _stream_agent_events(task_id: str, after_id: int, context: dict[str, str] | None = None) -> AsyncIterator[str]:
    cfg = get_settings()
    ...
    try:
        while True:
            records = store.list_agent_records(task_id=task_id, after_id=next_after_id, limit=200)
            for rec in records:
            ...
            if not task:
                ...
                logger.info(
                    "agent_events.stream.terminate task_id=%s reason=missing_task last_seq=%s delivered=%s",
                    task_id,
                    next_after_id,
                    delivered,
                )
            ...
            # Stop when the task is terminal and the latest poll had no new
            # public AgentEvent records.
            if last_task_status in TERMINAL_TASK_STATUSES and not records:
                logger.info(
                    "agent_events.stream.terminate task_id=%s reason=terminal task_status=%s last_seq=%s delivered=%s",
                    task_id,
                    last_task_status,
                    next_after_id,
                    delivered,
        ...
        logger.info(
            "agent_events.stream.disconnected task_id=%s requested_after_id=%s last_seq=%s task_status=%s delivered=%s",
            task_id,
            after_id,
            next_after_id,
            last_task_status,
        ...
        logger.info(
            "agent_events.stream.close task_id=%s requested_after_id=%s last_seq=%s task_status=%s delivered=%s",
            task_id,
            after_id,
            next_after_id,
            last_task_status,
