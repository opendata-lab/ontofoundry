"""Pi outcome -> TaskExecutionResult translation."""
...
from dataagent_backend.config import get_settings, update_settings  # noqa: E402
from dataagent_backend.core import task_executor  # noqa: E402
from dataagent_backend.core.pi_runtime import PiRunOutcome  # noqa: E402
from dataagent_backend.core.task_executor import TaskExecutionInput  # noqa: E402
...
def _params(**overrides: Any) -> TaskExecutionInput:
    defaults: dict[str, Any] = dict(
        ...
        resume_session_id="legacy-session-ignored-by-pi",
...
async def _run_adapter(monkeypatch, tmp_path: Path, outcome: PiRunOutcome, captured: dict[str, Any]):
    ...
    monkeypatch.setattr(
        "dataagent_backend.core.mcp_admin_service.get_runtime_registry_store",
        lambda: type("_EmptyRegistry", (), {"list_mcp_servers": lambda self: []})(),
    )
    monkeypatch.setattr("dataagent_backend.core.pi_runtime.resolve_cell_command", lambda cfg=None: ["/bin/true"])

    async def _fake_execute(ctx, *, writer, cancel_reason=None, **kwargs):
        captured["ctx"] = ctx
        return outcome

    monkeypatch.setattr("dataagent_backend.core.pi_runtime.execute_pi_run", _fake_execute)

    async def _no_cancel():
        return None

    return await task_executor._execute_pi(
        _params(),
        provider_id="anthropic",
        model="test-model",
        system_prompt="sys",
...
async def test_session_id_is_task_scoped(monkeypatch, tmp_path: Path):
    """The synthetic execution session id remains task scoped."""
    captured: dict[str, Any] = {}
    result = await _run_adapter(
...
async def test_missing_pi_runtime_reports_error_instead_of_raising(monkeypatch, tmp_path: Path):
    from dataagent_backend.core.pi_runtime import PiRuntimeUnavailable
    ...
    monkeypatch.setattr("dataagent_backend.core.pi_runtime.resolve_cell_command", _unavailable)

    async def _no_cancel():
        return None

    result = await task_executor._execute_pi(
        _params(),
        provider_id="anthropic",
        model="test-model",
        system_prompt="sys",
    ...
    # Surfaced through the shared error record, as with other runtime failures.
    # a missing execution engine.
...
async def test_mcp_servers_and_history_forwarded(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("dataagent_backend.core.mcp_admin_service.get_runtime_registry_store", lambda: FakeRegistry())
