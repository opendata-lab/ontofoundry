...
from dataagent_backend.core.topic_task_store import (
    TopicTaskStore,
    WIDGET_EVENT_TYPES,
...
def test_predicate_auth_disabled_is_legacy_byte_identical(monkeypatch):
    monkeypatch.setattr("dataagent_backend.core.topic_task_store.is_auth_enabled", lambda: False)
    store = TopicTaskStore()
...
def test_predicate_auth_enabled_matrix(monkeypatch):
    monkeypatch.setattr("dataagent_backend.core.topic_task_store.is_auth_enabled", lambda: True)
    store = TopicTaskStore()
...
def test_predicate_widget_branch_unchanged_regardless_of_auth(monkeypatch):
    ...
    for enabled in (True, False):
        monkeypatch.setattr("dataagent_backend.core.topic_task_store.is_auth_enabled", lambda enabled=enabled: enabled)
