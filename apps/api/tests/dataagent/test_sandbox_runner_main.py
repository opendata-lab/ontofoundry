...
import dataagent_backend.sandbox_runner_main as sandbox_runner_main
from dataagent_backend.config import get_settings, update_settings
from dataagent_backend.core.task_executor import TaskExecutionInput
from dataagent_backend.core.task_executor import TaskExecutionResult
...
def test_sandbox_runner_streams_records_and_result(monkeypatch):
    async def fake_execute(params, *, emit, is_cancel_requested=None):
        ...
        await emit({"record_type": "agent_event", "event_type": "run.started", "data": {"run_id": "run-1", "status": "runner"}})
        return TaskExecutionResult(
            task_status="finished",
            content="runner-api-ok",
            provider_id="openrouter",
            model="anthropic/claude-sonnet-4.5",
            session_id="pi-session-api",
        )
    ...
    assert lines[-1]["result"]["session_id"] == "pi-session-api"
...
def test_sandbox_runner_task_log_captures_child_runtime_stream_stderr(tmp_path: Path):
    log_path = tmp_path / "topic-1" / "logs" / "task-1.log"

    async def scenario() -> None:
        reader = asyncio.StreamReader()
        reader.feed_data(
            b"runtime_stream.thinking_repeated_segment task_id=task-1 topic_id=topic-1 repeat_count=3\n"
        )
        reader.feed_eof()
    ...
    assert "stderr runtime_stream.thinking_repeated_segment" in text
...
def test_sandbox_runner_container_command_mounts_only_topic_workspace(monkeypatch, tmp_path: Path):
    ...
    originals = {
        ...
        "dataagent_host_root": settings.dataagent_host_root,
    ...
    assert not any(value.startswith(OLD_SANDBOX_ROOT_ENV_PREFIX) for value in env_values)
...
print(json.dumps({
    ...
    "record": {
        ...
        "data": {"turn_id": "turn-1", "content_id": "c-1", "delta": large_payload},
    },
}), flush=True)
print(json.dumps({
    "type": "result",
...
"""
    ...
    assert emitted[0]["data"]["delta"] == "x" * (96 * 1024)
    assert kill_calls == []
...
def test_sandbox_runner_persists_child_logs_after_container_exit(monkeypatch, tmp_path: Path):
    ...
    host_root = tmp_path / "dataagent_runtime"
    sandbox_runner_main.CANCELLED_TASK_IDS.discard("task-1")
...
"""
    ...
    async def emit(record: dict) -> None:
        raise AssertionError(f"unexpected AgentEvent record: {record}")
...
def test_resolve_host_skills_dir_uses_auto_discovered_runner_mount(monkeypatch):
...
def test_sandbox_runner_requires_auto_discovered_host_skills_dir_when_agent_enables_skills(monkeypatch, tmp_path: Path):
    settings = get_settings()
    originals = {
        "dataagent_sandbox_backend": settings.dataagent_sandbox_backend,
        "dataagent_sandbox_image": settings.dataagent_sandbox_image,
    ...
    monkeypatch.setattr(sandbox_runner_main, "_AUTO_HOST_SKILLS_DIR", "")
    try:
        try:
            ...
            assert "host skills auto-discovery is unavailable" in str(exc)
        else:
            raise AssertionError("expected missing auto-discovered host skills dir to fail")
    finally:
        update_settings(originals)
...
def test_resolve_host_skills_dir_returns_empty_when_auto_discovery_missing(monkeypatch):
    monkeypatch.setattr(sandbox_runner_main, "_AUTO_HOST_SKILLS_DIR", "/auto/skills")
    assert sandbox_runner_main._resolve_host_skills_dir(get_settings()) == "/auto/skills"
    monkeypatch.setattr(sandbox_runner_main, "_AUTO_HOST_SKILLS_DIR", "")
