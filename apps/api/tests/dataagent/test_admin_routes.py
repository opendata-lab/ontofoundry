...
import dataagent_backend.api.admin_routes as admin_routes
from dataagent_backend.main import app
...
def test_agent_profile_routes_contract(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda documents: {"marketing-insights"},
    )
...
def test_agent_slash_commands_route_contract(monkeypatch):
    ...
    response = client.get("/api/v1/dataagent/agents/agent_1/slash-commands")
    assert response.status_code == 200
    assert response.json() == {
        "slash_commands": ["marketing-insights", "platform-tools"],
        "source": "profile",
    }
...
def _enable_auth(monkeypatch, tmp_path):
    import bcrypt
    import dataagent_backend.core.auth as auth
